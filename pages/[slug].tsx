...
const PostPage: React.FC<PostPageProps> = ({ content, frontMatter, prevPost, nextPost }) => {
    return (
        <PostLayout
            image={frontMatter && frontMatter.metadata ? frontMatter.metadata.image : ''}
        ...
        >
            ...
            {prevPost && (
                <Pagination>
                    <PrevPagination href={`/${prevPost.slug}`}>{prevPost.title}</PrevPagination>
                </Pagination>
            )}
            {nextPost && (
                <Pagination>
                    <NextPagination href={`/${nextPost.slug}`}>{nextPost.title}</NextPagination>
                </Pagination>
            )}
        </PostLayout>
    );
